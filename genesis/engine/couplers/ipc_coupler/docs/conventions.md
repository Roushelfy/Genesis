# IPC-Authoritative Coupling Mode — Conventions & Validation Ladder

Enforceable rules for the `ipc_authoritative` coupling type, and the tests that
...
1. **No Genesis integration.** `kernel_predict_integrate` is not called for any
   `ipc_authoritative` entity's DOFs. Genesis produces control torque only; IPC
   integrates dynamics.
2. **No Genesis contact/constraint solve.** When any `ipc_authoritative` entity
...
6. **Integrator is BDF1.** The mode rejects (asserts) a non-BDF1 scene
   integrator.
7. **Fixed base only (v1).** A non-fixed base routed to `ipc_authoritative` is a
...
8. **Revolute + fixed joints only (v1).** Any other joint type on an
   `ipc_authoritative` entity is a build-time error.
9. **Single env (v1).** `B>1` with `ipc_authoritative` is rejected until the
...
- Enum: `COUPLING_TYPE.IPC_AUTHORITATIVE` (`data.py`).
- User selector: `RigidMaterial(coup_type="ipc_authoritative")`.
- Option: `IPCCouplerOptions.actuation_mode` (string).
- New coupler methods: `_add_ipc_authoritative_entities`, `_push_joint_torques`,
...
| **Contract** | building `ipc_authoritative` with free base / non-revolute joint / BDF2 / B>1 raises; building it flips `enable_collision`/`disable_constraint` off | Rules 2,6,7,8,9 |
| **Source-scan** | the `ipc_authoritative` path contains no `kernel_predict_integrate`, no `ExternalArticulationConstraint`, no motor/driving-joint constitution | Rules 1,3 |
