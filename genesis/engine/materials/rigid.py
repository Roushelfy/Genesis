...
from genesis.typing import NonNegativeFloat, PositiveFloat, PositiveVec2FType, StrictInt, StrArrayType, ValidFloat
...
CoupType = Literal["two_way_soft_constraint", "external_articulation", "ipc_only", "ipc_monolithic"]
...
class Rigid(Kinematic["RigidEntity"]):
    ...
    contact_d_hat : float or None, optional
    ...
    coup_stiffness: PositiveVec2FType = (100.0, 100.0)

    @model_validator(mode="before")
    @classmethod
    def _resolve_defaults(cls, data: dict) -> dict:
        # ipc_only / ipc_monolithic entities have their dynamics fully controlled by IPC
        # (gravity + collision). Genesis gravity must be disabled to avoid double-counting.
        if data.get("coup_type") in ("ipc_only", "ipc_monolithic"):
