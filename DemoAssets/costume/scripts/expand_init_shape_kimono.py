...
``DemoAssets/kimono/``, the run ``output`` dir (default ``scripts/results``), then
``scripts/results/v1/``. Loading runs **after** ``setup_scene`` (defined in this file) and **before**
...
def try_load_kimono_v0_saved_config(
    ...
    Search order per file: ``kimono_dir`` → ``results_dir`` → ``scripts/results/v1``.
