...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="MAX_FRAME",
        help="Recover mode: replay dumped frames and export surface OBJs. "
        "Optionally specify max frame number (default: recover all).",
    )
...
def discover_kimono_pieces(kimono_dir: Path) -> list[ClothPiece]:
    ...
    for init_path in init_paths:
        ...
        if orig_name in SKIP_CLOTH_PIECES:
            print(f"[kimono] skipping {base} (in SKIP_CLOTH_PIECES)")
            continue
