...
STC_STRENGTH_SUN     = np.array([100, 100], dtype=np.float64)  # sun: driven rotation + position
STC_STRENGTH_RING    = np.array([1000, 0],  dtype=np.float64)  # ring: locked translation, free rotation
STC_STRENGTH_PLANET  = np.array([100, 100], dtype=np.float64)  # planet_0: pick-and-place
SUN_RPM          = -15.0
SUN_RAD_PER_S    = SUN_RPM * 2 * np.pi / 60.0
...
SETTLE_TIME      = 0.3        # seconds to wait after release before sun starts
SUN_RAMP_TIME    = 2.0        # seconds to ramp up sun speed from 0 to full
...
def main():
    ...
    # ── Part definitions: (name, obj_file, is_fixed, is_driven) ──
    parts = [
        ("sun_gear",  "sun_gear.obj",  False, True),   # STC driven rotation
        ("ring_gear", "ring_gear.obj", False, True),   # STC: free rotation, locked translation
        ("carrier",   "carrier.obj",   True,  False),  # fully fixed
    ]
    ...
    for name, obj_file, is_fixed, is_driven in parts:
        ...
        if is_driven:
            if name == "sun_gear":
                strength = STC_STRENGTH_SUN
            elif name == "ring_gear":
                strength = STC_STRENGTH_RING
    ...
    t_sun_start = t_phase1 + SETTLE_TIME    # sun starts after settle
    ...
    print(f"  [ANIM] Phase 2: sun rotation starts at t={t_sun_start:.2f}s (settle {SETTLE_TIME:.1f}s)")
    ...
    def sun_animation(info: Animation.UpdateInfo):
        geo: SimplicialComplex = info.geo_slots()[0].geometry()
        is_constrained = view(geo.instances().find(builtin.is_constrained))
        aim_transform = view(geo.instances().find(builtin.aim_transform))
        t = info.frame() * info.dt()
        ...
        if t < t_sun_start:
            ...
            dt = t - t_sun_start
            if dt < SUN_RAMP_TIME:
                frac = dt / SUN_RAMP_TIME
                angle = SUN_RAD_PER_S * 0.5 * frac * dt
            else:
                angle_ramp = SUN_RAD_PER_S * 0.5 * SUN_RAMP_TIME
                angle = angle_ramp + SUN_RAD_PER_S * (dt - SUN_RAMP_TIME)
            aim_transform[0] = _rotation_z_4x4(angle)

    animator.insert(sun_obj, sun_animation)

    def ring_animation(info: Animation.UpdateInfo):
        geo: SimplicialComplex = info.geo_slots()[0].geometry()
        is_constrained = view(geo.instances().find(builtin.is_constrained))
        aim_transform = view(geo.instances().find(builtin.aim_transform))
        is_constrained[0] = 1
        aim_transform[0] = np.eye(4, dtype=np.float64)

    animator.insert(ring_obj, ring_animation)
    ...
    def gui_callback():
        ...
        imgui.TextUnformatted(f"Sun RPM: {SUN_RPM}  "
                              f"d_hat: {D_HAT}m")
