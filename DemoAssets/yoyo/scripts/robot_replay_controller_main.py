...
_STRING_OBJ = _SCRIPT_DIR.parent / "v5" / "yoyo_string_20036.obj"
...
def run(
    ...
    # ── UIPC scene setup (always, like run_inspector) ──────────────────
    app = URDFGuiApp(
        urdf_path=urdf_path,
    ...
    # Main controller uses collision meshes for IPC bodies
    ...
    def on_frame(info: Any, ctrl: URDFController) -> None:
        s = app._sim_ctx.get("schedule", schedule)
    ...
    if has_ipc_bodies:
        ...
        print("[replay] WARNING: No IPC bodies created (sim link patterns matched nothing). "
              "Simulation will not drive the robot — FK scrubbing only.")
    ...
    app._sgui = sgui
    app._sio = sio
    app._sim_ctx["sgui"] = sgui
    app._sim_ctx["sio"] = sio

    # Non-sim collision nodes as wireframe overlay
    controller.register_visual_only_meshes(transparency=0.35, edge_width=0.5,
                                           color=(0.2, 0.9, 0.4))
    controller.update_visual_meshes()

    # ── Visual mesh overlay for display ────────────────────────────────
    ...
    # Trajectory yoyo overlays (for FK scrubbing, before sim starts)
    # ── Reference OBJ state ────────────────────────────────────────────
    ...
    sim = {
        ...
        "total_frames": total_schedule_frames,
        "recover_frame": 0,
    }
    ...
    def _update_stitch() -> None:
        if stitch_net is not None:
            result = app.scene_state.build_stitch_line_nodes()
            if result is not None:
                stitch_net.update_node_positions(result[0])
    ...
    def _draw_sim_panel() -> None:
        ...
        imgui.Text(f"UIPC dt: {sim_dt:.6f}s  |  Traj dt: {traj.dt:.6f}s  |  Speed: {play_speed}x")

        if not sim["active"]:
            imgui.Text(f"Trajectory: {sim['total_frames']} frames ready.")
            if not has_ipc_bodies:
            ...
            if sim.get("replaying"):
                ...
                steps_this_tick = min(sim["steps_per_tick"], replay_target - world.frame())
                for _ in range(max(1, steps_this_tick)):
                    next_f = world.frame() + 1
                    if world.recover(next_f):
                        world.retrieve()
                        if sim["export_surface"]:
                            sio.write_surface(f"{output_dir}/surface_{world.frame()}.obj")
                    else:
    ...
    on_update._refs = (engine, world, scene, sgui, sio, controller, app)
...
def run_export_recover(
    ...
    """Recover UIPC dump frames and export seq data (NPY) for Genesis replay.

    Recovers only every ``frame_skip``-th dump frame directly (skipping
    intermediate frames), so only max_frame/frame_skip recovers are needed.
    """
    from uipc import view
    ...
    targets = list(range(frame_skip, max_frame + 1, frame_skip))
    print(f"[export-recover] will recover {len(targets)} frames "
          f"(every {frame_skip}-th from {frame_skip} to {targets[-1] if targets else 0})")
    ...
    for target in targets:
        ...
        if exported % 100 == 0:
            ...
            print(f"[export-recover] {exported}/{len(targets)} frames captured "
                  f"({elapsed:.1f}s elapsed)")

    exporter.save(seq_dir)
    elapsed = time.time() - t_start
    print(f"[export-recover] Done: {exported} frames (skip={frame_skip}) "
          f"in {elapsed:.1f}s -> {seq_dir}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="No-GUI: simulate MAX_FRAME steps, export seq NPY, then exit.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Override sequence export directory (default: DemoAssets/yoyo/v6/seq).",
    )
    args = parser.parse_args()

    urdf_path = str(_REPO_ROOT / "DemoAssets" / "marvin_sharpa_description" / "marvin_sharpa.urdf")
