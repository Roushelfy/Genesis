#!/usr/bin/env python3
"""
Apply (x,y,z) → (z,x,y) rotation to all STL files in hands/left/,
save as *_modified.stl, and create left_modified.urdf with transformed
...
def transform_urdf(input_urdf: Path, output_urdf: Path):
    ...
    for joint in root.findall(".//joint"):
        ...
        if origin is not None:
            ...
            nz, nx, ny = rotate_xyz(x, y, z)
            origin.set("xyz", f"{nz} {nx} {ny}")
        ...
        if axis_elem is not None:
            ...
            if abs(ax) > 1e-15 or abs(ay) > 1e-15 or abs(az) > 1e-15:
                naz, nax, nay = rotate_xyz(ax, ay, az)
                axis_elem.set(
                    "xyz",
...
def main():
    ...
    # Transform URDF
    input_urdf = base_dir / "left.urdf"
    output_urdf = base_dir / "left_modified.urdf"
