...
-----------------------------
  ...
  are absolute world transforms stored as ``[px, py, pz, qw, qx, qy, qz]``
  (Genesis convention: **scalar-first** quaternion).
...
# Use the OBJ-origin convention trajectory (converted via convert_trajectory.py).
# Uniform rendering formula for ALL bodies:  V_world = R(q) @ V_scaled + pos
...
_GEAR_FILES: dict[str, str] = {
    ...
    "carrier":       "carrier.obj",
    "planet_gear_0": "planet_gear_v2.obj",
    "planet_gear_1": "planet_gear_v2.obj",
    "planet_gear_2": "planet_gear_v2.obj",
}
...
def apply_world_transform(V_scaled: np.ndarray, pos_quat: np.ndarray) -> np.ndarray:
    """V_world = R(q) @ V_scaled + pos.  Uniform formula for all bodies."""
...
def build_gear_body_local_verts(V_obj_mm: np.ndarray) -> np.ndarray:
    """V_scaled = V_obj_mm * MESH_SCALE. That's it."""
...
def _build_qpos_joint_order(ctrl) -> list[str]:
    ...
    Loaded from ``genesis_joint_order.json`` (generated by running Genesis
    with the same URDF and printing ``robot.joints`` in qpos order).
    Falls back to ctrl.joint_names (URDF declaration order) if the file
    is not found — which is WRONG for marvin_sharpa but at least won't crash.
    """
...
def main() -> None:
    ...
    for name, obj_file in _GEAR_FILES.items():
        ...
        gear_verts_local[name] = build_gear_body_local_verts(V)
        gear_faces[name] = F
    ...
    def gui_callback() -> None:
        ...
        # --- Gear debug info ---
        imgui.Separator()
        opened = imgui.CollapsingHeader("Gear / Robot debug")
        if opened:
            p = rigid_poses["sun_gear"][frame[0]]
            qw, qx, qy, qz = p[3].astype(float), p[4].astype(float), p[5].astype(float), p[6].astype(float)
            r22 = float(1 - 2*(qx*qx + qy*qy))
            import math
            tilt = math.degrees(math.acos(max(-1.0, min(1.0, r22))))
            imgui.Text(f"sun_gear  pos=({p[0]:.3f},{p[1]:.3f},{p[2]:.3f})  tilt={tilt:.1f}deg")
            p2 = rigid_poses["ring_gear"][frame[0]]
            imgui.Text(f"ring_gear pos=({p2[0]:.3f},{p2[1]:.3f},{p2[2]:.3f})")
            if robot_ctrl is not None:
                tf7 = robot_ctrl.get_link_transform("Link7_R")
                w7 = tf7[:3, 3]
                imgui.Text(f"Link7_R   pos=({w7[0]:.3f},{w7[1]:.3f},{w7[2]:.3f})")
