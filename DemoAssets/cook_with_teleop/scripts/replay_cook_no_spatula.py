"""UIPC scene: replay cooking teleop with the SPATULA COLLISIONS DISABLED.

Variant of ``replay_cook.py`` that keeps the spatula in the scene (it still
follows the teleop trajectory visually), but turns OFF every contact pair
that involves the spatula.  The simulation reduces to the pan + food only
(noodles / broccoli / tomato / mushroom interact with each other and with
the pan), so you get a much faster, simpler IPC solve when the spatula
isn't relevant to the experiment.
...
Usage:
    python replay_cook_no_spatula.py [--no-gui] [--frames N] [--traj PATH] [--speed 0.4]
    # Default --export-seq is "../seq_no_spatula" so a sequence is always
    # exported there; pass --export-seq "" to skip exporting.
    python replay_cook_no_spatula.py --export-seq ../seq_no_spatula
    python replay_cook_no_spatula.py --export-recover 5000  # export from dump
"""
...
def main():
    ...
    parser.add_argument("--export-seq", type=str, default=str(_ASSET_ROOT / "seq_no_spatula"),
                        help="Directory to export sequence for Genesis replay "
                             "(default: <cook_with_teleop>/seq_no_spatula).  "
                             "Pass an empty string to disable export.")
    ...
    # ---- Contact tabular: ALL spatula collisions DISABLED ----------------
    # Variant of replay_cook.py for "pan + food only" experiments.  The
    # spatula stays kinematically driven by the teleop trajectory but does
    # NOT collide with anything; every pair involving spatula_contact is
    # registered with enable=False so the IPC solver skips those checks.
    ...
    pan_friction = 0.01
    # ---- Spatula contacts: DISABLED ----
    tabular.insert(pan_contact,      spatula_contact, 0.0, 1.0 * GPa, enable=False)
    tabular.insert(broccoli_contact, spatula_contact, 0.5, 1.0 * GPa, enable=False)
    tabular.insert(noodle_contact,   spatula_contact, 0.5, 1.0 * GPa, enable=False)
    tabular.insert(tomato_contact,   spatula_contact, 0.5, 1.0 * GPa, enable=False)
    tabular.insert(mushroom_contact, spatula_contact, 0.5, 1.0 * GPa, enable=False)

    # ---- Food <-> pan / food <-> food: ENABLED ----
    ...
    if args.export_recover >= 0:
        if seq_dir is None:
            seq_dir = _ASSET_ROOT / "seq_no_spatula"
        world.retrieve()
