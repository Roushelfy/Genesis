...
class WearingRuntimeAPI:
    def __init__(
        ...
        self._bone_base_starts = [
            np.asarray(proxy.config.start, dtype=np.float64).reshape(3).copy() for proxy in self.core.proxies
        ]
        self._bone_base_ends = [
            np.asarray(proxy.config.end, dtype=np.float64).reshape(3).copy() for proxy in self.core.proxies
        ]
