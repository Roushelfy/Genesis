...
def _make_box_trimesh(sx: float, sy: float, sz: float, tx: float = 0.0, ty: float = 0.0, tz: float = 0.0):
    """Create a box as a SimplicialComplex with half-extents sx,sy,sz and center offset tx,ty,tz."""
...
box_mesh = _make_box_trimesh(box_width, box_height, box_depth)
box_mesh = process_surface(box_mesh)

handle_mesh = _make_box_trimesh(
    box_width / 2.0,
    handle_height * 4,
    box_depth / 2.0,
    ty=handle_height * 2,
...
# nut_base_mesh = _make_box_trimesh(1.0, 1.0, 1.0)
