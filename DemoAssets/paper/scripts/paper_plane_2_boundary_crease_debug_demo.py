...
def resolve_origami_asset_path(asset_filename: str | None = None) -> str:
    ...
    if os.path.exists(asset_filename):
        return asset_filename
    return os.path.join(AssetDir.asset_path(), asset_filename)
...
def collect_crease_candidates(
    ...
    for signature, edges in line_edges.items():
        ...
        boundary_line_vertices = [
            ...
            if vid in boundary_vertices and classify_boundary_side(plane_positions[vid], bbox) != "interior"
        ]
        if len(boundary_line_vertices) != 2:
            continue

        projections = {vid: float(np.dot(plane_positions[vid], direction)) for vid in line_vertices}
...
def order_component_path(
    ...
    local_degree = {vid: sum(1 for nb in adjacency[vid] if nb in component_set) for vid in component}
...
def export_label_map(
    ...
    try:
        ...
        raise SystemExit("Label-map export requires matplotlib. Install it with `pip install matplotlib`.") from exc

    output = Path(output_path) if output_path is not None else default_label_map_path(diagnostics["asset_filename"])
...
def run_demo(asset_filename: str | None = None):
    ...
    def on_update():
        ...
        psim.Text(f"Plane axes: {plane_axes[0]}, {plane_axes[1]}  normal: {diagnostics['normal_axis_name']}")
        psim.Text(f"Oscillation amplitude: {OSCILLATION_AMPLITUDE:.4f}")
        psim.Text(f"Detected creases: {len(diagnostics['chains'])}")
        psim.Text("Alias rule: BT_V* verticals, LR_H* horizontals, DP/DN positive/negative diagonals")
        psim.Separator()
