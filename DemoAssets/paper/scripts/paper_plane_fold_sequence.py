...
Run:
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py --sequence full --no-from-start
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py --bending-model stress
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py DemoAssets/paper/paper_plane_2_fine.obj
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py --start-after-step 2
    python DemoAssets/paper/scripts/paper_plane_fold_sequence.py --overhead-big-cube
"""
...
def build_step_specs(context: AssetContext, sequence: str = "full") -> tuple[StepSpec, ...]:
    boundary_step6_crease = (
        "C04_UPPER_SHORT" if "paper_plane_5" in Path(context.asset_filename).stem.lower() else "C01_C04_X_TO_C03_C04_X"
    )
    step1 = StepSpec(
    ...
    step4 = StepSpec(
        ...
        strong_fix_labels=("LEFT_BOTTOM", "RIGHT_BOTTOM", "P09_C02_X_TO_P08_P13_X_MID", "P06A", "P08_P13_X"),
    ...
    if sequence == "short":
        return (
            ...
            step7,
            step8,
            step_c01_c04_to_p01a,
            step7,
            step_c03_c04_to_p03a,
            step_c03_c04_to_p03a_no_stamp,
        )
...
def build_demo(
    ...
    *,
    ...
    max_press_path_points = max(
        max(len(step.press.path_labels), 2) if step.press is not None else 2 for step in schedule_steps
    )
...
def run_demo(
    ...
    *,
    ...
    def on_update():
        ...
        psim.Text(f"Cube home: ({cube_home_xyz[0]:+.4f}, {cube_home_xyz[1]:+.4f}, {cube_home_xyz[2]:+.4f})")
        if resolved_step.global_flip is not None:
            psim.Text(
        ...
        for mover in resolved_step.movers:
            ...
            psim.Text(
                f"{mover.label:<12s} v{mover.vertex_id:<5d} -> {mover.target_label:<16s} v{mover.target_vertex_id:<5d}"
            )
            psim.Text(f"start=({sx:+.4f}, {sy:+.4f}, {sz:+.4f})  target=({tx:+.4f}, {ty:+.4f}, {tz:+.4f})")
