...
ROUNDED_CUBE_MESH_PATH = os.path.join(AssetDir.asset_path(), "sim_data", "trimesh", "rounded_cube.obj")
PLAIN_CUBE_MESH_PATH = os.path.join(AssetDir.asset_path(), "sim_data", "trimesh", "cube.obj")
...
def motion_schedule(
    ...
    for segment_index in range(len(press_points) - 1):
        segment_frames = PRESS_TO_LEFT_FRAMES if len(press_points) >= 3 and segment_index == 0 else PRESS_TO_RIGHT_FRAMES
...
def maybe_recover_world(
    ...
    if start_after_step is not None:
        ...
        if start_after_step < 0 or start_after_step > len(schedule_steps):
            raise SystemExit(
                f"--start-after-step must be between 0 and {len(schedule_steps)}, got {start_after_step}."
            )
    ...
    next_name = schedule_steps[next_index].name if next_index is not None else "inspect"
    print(
        f"[paper_plane_resume] recovered from frame {dump_frame} "
        f"after step {manifest.get('last_completed_step_name')}"
    )
    resume_info.update(
        {
            "mode": "recovered",
...
def default_cube_mesh_path() -> str:
    if os.path.exists(PLAIN_CUBE_MESH_PATH):
...
def register_boundary_loop_segment_crease(
    ...
    *,
    ...
    if prefer == "lower":
        ...
        segment_ids = segment_a if polyline_length(segment_a) <= polyline_length(segment_b) else list(reversed(segment_b))
...
def choose_upper_signed_angle(
    ...
    if math.isclose(abs(signed_angle), math.pi, abs_tol=1.0e-6):
        probe_plus = center_xyz + 0.5 * (axial_offset_start + axial_offset_end) * axis_dir + 0.5 * (radius_start + radius_end) * (
...
def build_global_flip_action(
    ...
    if mover_labels is None:
        ...
        mover_vertex_ids = tuple(
            sorted({int(named_vertices[label].vertex_id) for label in mover_labels})
        )
...
def compile_step(
    ...
    for label in step_spec.strong_fix_labels:
        ...
        _add_anchor(strong_fixes, strong_fix_ids, vertex.vertex_id, label, vertex.xyz, current_pose_y=STRONG_FIX_CURRENT_Y)
    ...
    for start_label, end_label in step_spec.strong_fix_segment_labels:
        segment_ids = sample_segment_vertex_ids(
            positions, named_vertices, start_label, end_label, spacing=0.25 * CUBE_SCALE,
        )
    ...
    if not passive_step:
        for mover_label in step_spec.mover_labels:
            ...
            if target_distance <= TARGET_SNAP_TOL:
                ...
                _add_anchor(strong_fixes, strong_fix_ids, target.vertex_id, target.label, target.xyz, lock_current_pose=False)
            else:
                target = NamedVertex(
