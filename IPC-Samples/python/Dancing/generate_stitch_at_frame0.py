...
def _closest_on_tri(pt, v0, v1, v2):
    ab = v1 - v0; ac = v2 - v0; ap = pt - v0
    d1, d2 = ab @ ap, ac @ ap
    if d1 <= 0 and d2 <= 0: return v0.copy()
    bp = pt - v1; d3, d4 = ab @ bp, ac @ bp
    if d3 >= 0 and d4 <= d3: return v1.copy()
    cp = pt - v2; d5, d6 = ab @ cp, ac @ cp
    if d6 >= 0 and d5 <= d6: return v2.copy()
    vc = d1 * d4 - d3 * d2
    if vc <= 0 and d1 >= 0 and d3 <= 0: return v0 + d1 / (d1 - d3) * ab
    vb = d5 * d2 - d1 * d6
    if vb <= 0 and d2 >= 0 and d6 <= 0: return v0 + d2 / (d2 - d6) * ac
    va = d3 * d6 - d5 * d4
...
def main():
    ...
    for (na, nb), pairs_np in all_pairs.items():
        ...
        for i in range(n):
            vid = pairs_np[i, 0]; tid = pairs_np[i, 1]
            pt = va[vid]; tri = fb[tid]
            proj = _closest_on_tri(pt, vb[tri[0]], vb[tri[1]], vb[tri[2]])
            nodes[2 * i] = pt; nodes[2 * i + 1] = proj
            edges[i] = [2 * i, 2 * i + 1]
        ...
        for i in range(n):
            node_dist[2 * i] = dists[i]; node_dist[2 * i + 1] = dists[i]
