...
class URDFController:
    ...
    def _collect_mesh_nodes(self) -> list[MeshNode]:
        link_names = {j.parent_link for j in self._joints.values()} | {
            j.child_link for j in self._joints.values()
        }
        nodes: list[MeshNode] = []
    ...
    def _node_matches(self, node: MeshNode, patterns: list[str]) -> bool:
        ...
        return any(
            fnmatch.fnmatch(raw, p) or fnmatch.fnmatch(base, p) or fnmatch.fnmatch(link, p)
            for p in patterns
        )
    ...
    def create_ipc_bodies(
        ...
        for i, node in enumerate(self._mesh_nodes):
            ...
            if inertial is not None:
                mass_12x12 = affine_body.from_rigid_body(
                    inertial.mass, inertial.center_of_mass, inertial.inertia
                )
            ...
            self._bindings.append(
                BodyBinding(
                    node_name=node.node_name, object_name=obj_name,
                    parent_link=node.parent_link,
                    obj=obj, geo_slot=geo_slot, rest_geo_slot=rest_geo_slot,
                )
            )
    ...
    def register_visual_only_meshes(
        ...
        for node in self._mesh_nodes:
            ...
            m = ps.register_surface_mesh(
                f"visual_{node.node_name}", node.local_vertices, node.faces
            )
    ...
    def _build_ik_chain(
        ...
        for jname in chain_joints:
            ...
            if info.type in ("revolute", "continuous"):
                ...
                is_active = (
                    jname in self._joint_state
                    if active_joints is None
                    else jname in active_joints
                )
            elif info.type == "prismatic":
