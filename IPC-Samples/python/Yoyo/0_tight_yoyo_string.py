...
string_obj = folder / "results" / "yoyo-string3.obj"
...
AffineBodyConstitution().apply_to(bearing_mesh, 200.0 * MPa)
bearing_contact.apply_to(bearing_mesh)
view(bearing_mesh.instances().find(builtin.is_fixed))[0] = 1
scene.objects().create("yoyo_bearing").geometries().create(bearing_mesh)


string_mesh = io.read(str(string_obj))
label_surface(string_mesh)
HookeanSpring().apply_to(string_mesh, 1 * GPa, thickness=0.0002, mass_density=100.0)
