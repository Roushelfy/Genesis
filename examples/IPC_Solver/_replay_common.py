...
class TrajectoryReplay:
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "--dark-bg", action="store_true", help="True-black background (pure_black.exr, multiplier=0) — Nyx only"
        )
        parser.add_argument(
            "--preview",
            action="store_true",
    ...
    def nyx_env_map(self) -> tuple:
        """Return ``(exr_path, yaw_radians, multiplier)`` for the Nyx env map.

        Override in subclasses that pick a per-task EXR so Nyx matches the
        Luisa RayTracer's ``env_surface`` for the same task. Default is the
        legacy san_carlos studio HDRI used by every replay before the
        per-task table existed.
        """
        return (
    ...
    def _add_camera(self):
        ...
        if self.args.nyx:
            ...
            # Subclass picks the per-task EXR via nyx_env_map() so Nyx matches
            # the Luisa RayTracer's env_surface for the same task.
            env_path, env_yaw, env_multiplier = self.nyx_env_map()
            env_map.texture = str(env_path)
            env_map.rotation = float(env_yaw)
            env_map.multiplier = float(env_multiplier)
            if getattr(self.args, "dark_bg", False):
                # True-black background: pure_black.exr (all-zero RGB) with multiplier=0
                # so the env contributes neither light nor visible color.
                env_map.texture = str((_REPO_ROOT / "DemoAssets/textures/pure_black.exr").resolve())
                env_map.multiplier = 0.0

            # Subclasses can disable the env map by overriding nyx_use_env_map() → False
            use_env_map = getattr(self, "nyx_use_env_map", lambda: True)()
            ...
            nyx_kwargs = dict(
                ...
                env_maps=(env_map,) if use_env_map else (),
